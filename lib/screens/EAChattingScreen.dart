...
class EAChattingScreenState extends State<EAChattingScreen> {
  ...
  void initializeSocket() {
    socket = IO.io('${AppConstants.chatUrl}', <String, dynamic>{
  ...
  Future<void> fetchChatMessages() async {
    ...
    try {
      final response = await http.get(
        Uri.parse('${AppConstants.chatUrl}/api/messages/conversation/$conversationId'),
      );
    ...
    if (msgController.text.trim().isNotEmpty && conversationId != null && currentUserId != null) {
      ...
      try {
        final response = await http.post(
          Uri.parse('${AppConstants.chatUrl}/api/messages'),
  ...
  Future<void> markMessagesAsSeen() async {
    ...
    try {
      final response = await http.post(
        Uri.parse('${AppConstants.chatUrl}/api/messages/seen'),
        headers: {'Content-Type': 'application/json'},
        body: jsonEncode({
          'conversationId': conversationId,
