      ...
      if (response.statusCode == 200) {
        ...
        if (exists == false && message == "Email is registered but fullname is missing") {
          print("Condition met, showing profile update popup");
            ...
            final prefs = await SharedPreferences.getInstance();
          await prefs.setBool('profile_completed', false); // Mark profile as completed
  ...
  Future<void> _saveProfile() async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        final userId = responseData['data']['_id'] as String?; // Extract _id from response

        if (userId != null) {
          final prefs = await SharedPreferences.getInstance();
          await prefs.setString('user_id', userId); // Save _id to SharedPreferences
          await prefs.setBool('profile_completed', true); // Mark profile as completed
